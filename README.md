...
### クリップボードコピーの検証警告（開発者向け）

**症状**: ログに以下の警告が表示される

```text
WARNING - ⚠️ UTF-8デコードエラー - errors='replace'でデコードしました
WARNING - ⚠️ クリップボード内容が一致しません (expected: XXXXX, actual: YYYYY)
```

**原因**: v1.1.1以前のバージョンで、`pbpaste`による検証処理が誤検知していた

**解決済み**: v1.1.1改善版（2025-10-21）で検証処理を削除

**詳細**: [開発ログ](docs/development/20251021_clipboard_verification_noise_removal.md)

**注意**:

- この警告は「検証処理の失敗」であり、「クリップボードコピーの失敗」ではない
- AppleScript経由のコピーは正常に動作している
- ユーザー体験には影響なし（ログにのみ表示）
